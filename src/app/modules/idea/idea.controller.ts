...
const getDraftIdeas = catchAsync(async (req: Request, res: Response) => {
  const page = Number(req.query.page || 1);
  const limit = Number(req.query.limit || 12);
  const result = await ideaServices.getDraftIdeas(
    req.user as IRequestUser,
    page,
    limit,
  );
...
const getApprovedAndRejectedIdeas = catchAsync(
...
const upVote = catchAsync(async (req: Request, res: Response) => {
  const { ideaId } = req.params;
  const result = await ideaServices.upVote(
    req.user as IRequestUser,
    ideaId as string,
  );
  ...
  sendResponse(res, {
    success: true,
    statusCode: status.ACCEPTED,
    message: "Idea up voted successfully",
    data: result,
  });
});
const downVote = catchAsync(async (req: Request, res: Response) => {
  const { ideaId } = req.params;
  const result = await ideaServices.downVote(
    req.user as IRequestUser,
    ideaId as string,
  );
  sendResponse(res, {
    success: true,
    statusCode: status.ACCEPTED,
    message: "Idea down voted successfully",
    data: result,
  });
});
const votedIdea = catchAsync(async (req: Request, res: Response) => {
  const { ideaId } = req.params;
  const result = await ideaServices.votedIdea(
    req.user as IRequestUser,
    ideaId as string,
  );
  sendResponse(res, {
    success: true,
    statusCode: status.OK,
    message: "Idea voted successfully",
    data: result,
  });
});
